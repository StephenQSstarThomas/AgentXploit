...
"""
SWE-ReX integration tool for bash execution and docker deployment.
Provides functionality to execute bash commands and deploy to docker environments.
"""
...
class SWEReXTool:
    """
    SWE-ReX tool for bash execution and docker environment deployment.
    Supports both local and docker deployments with persistent bash sessions.
    """
    
    def __init__(self):
        self.deployments: Dict[str, Union[LocalDeployment, DockerDeployment]] = {}
    ...
    async def create_docker_deployment(
        ...
        **kwargs
        ...
        Args:
            ...
            deployment_id: Unique identifier for the deployment
            **kwargs: Additional arguments for DockerDeployment
            
        Returns:
            deployment_id: The ID of the created deployment
        """
        try:
            deployment = DockerDeployment(image=image, **kwargs)
            await deployment.start()
    ...
    async def create_openhands_deployment(
        ...
        **kwargs
        ...
        Uses the openhands-agent1 container.
        ...
        try:
            ...
            logger.info(f"OpenHands deployment '{deployment_id}' created successfully")
            return deployment_id
    ...
    async def execute_command(
        ...
        """
        Execute a one-off command (similar to subprocess.run()).
        ...
        try:
            if isinstance(command, str):
                # Convert string command to list for Command
                command_list = ["bash", "-c", command]
            else:
                command_list = command
            
            result = await runtime.execute(Command(command=command_list))
    ...
    async def create_bash_session(
        ...
        try:
            ...
            image = getattr(deployment_info, 'image', None) if hasattr(deployment_info, 'image') else None
    ...
    async def run_in_session(
        ...
        try:
            result = await runtime.run_in_session(BashAction(command=command))
            
            # BashObservation has 'output' instead of 'stdout'
            return ExecutionResult(
                stdout=result.output,
                stderr=getattr(result, 'stderr', result.failure_reason),
                exit_code=result.exit_code,
                success=result.exit_code == 0
            )
            
        except Exception as e:
            logger.error(f"Failed to run command in session: {e}")
            return ExecutionResult(
                stdout="",
                stderr=str(e),
                exit_code=-1,
    ...
    def list_deployments(self) -> Dict[str, Dict[str, Any]]:
        ...
        for deployment_id, deployment in self.deployments.items():
            deployment_type = type(deployment).__name__
            image = getattr(deployment, 'image', None) if hasattr(deployment, 'image') else None
...
async def execute_bash_command(
    ...
    """
    Execute a bash command using the global SWE-ReX tool instance.
...
async def create_docker_environment(
    ...
    """
    Create a docker environment using the global SWE-ReX tool instance.
...
async def create_local_environment(deployment_id: str = "local") -> str:
    """
    Create a local environment using the global SWE-ReX tool instance.
